...
class MyDataSet(Dataset.Dataset):#train数据
    def __init__(self,path):
        ...
        ncount = 0
        i = 0
        data = torch.zeros((1088, 3072))
        for fname in os.listdir(path):
            lpath = os.path.join(path, fname)
            ...
            ncount += 1
        pca_data = pca_1.fit_transform(data)
        print("pca_data.shape:")
        print(pca_data.shape)
        for j in range(918):
            self.images.append(pca_data[j])
